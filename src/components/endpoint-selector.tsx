...
export function EndpointSelector({ endpoints, selected, onSelect }: EndpointSelectorProps) {
	const isSelected = (endpoint: ParsedEndpoint) =>
		endpoint.path === selected.path && endpoint.method === selected.method;

	return (
		<div className="space-y-2">
			<h3 className="text-sm font-medium text-muted-foreground">Select endpoint</h3>
			<div className="flex flex-wrap gap-2">
				{endpoints.map((endpoint) => {
					...
					return (
						...
						>
							<Badge variant="outline" className={`mr-2 ${METHOD_COLORS[endpoint.method] || ''}`}>
								{endpoint.method}
							</Badge>
