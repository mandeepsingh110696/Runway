import type { GeneratedSnippet, ParsedEndpoint, SnippetFormat } from '@/types/openapi';
...
function generateFetch(options: SnippetOptions): GeneratedSnippet {
	...
	if (auth?.headerName && auth.headerValue) {
		...
		headers[auth.headerName] =
			auth.headerValue.replace(/\$/g, '${process.env.').replace(/([A-Z_]+)(?=})/, '$1') +
			(auth.headerValue.includes('$') ? '' : '');
	}
...
function generatePython(options: SnippetOptions): GeneratedSnippet {
	...
	const lines: string[] = ['import requests', 'import os', ''];
...
function getExampleBody(endpoint: ParsedEndpoint): Record<string, unknown> | null {
	...
	const jsonContent =
		endpoint.requestBody.content['application/json'] || endpoint.requestBody.content['*/*'];
...
function buildExampleFromSchema(schema: Record<string, unknown>): Record<string, unknown> | null {
	if (schema.example) {
