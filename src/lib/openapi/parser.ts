...
export async function parseOpenAPISpec(input: string): Promise<ParsedSpec> {
	...
	return transformSpec(spec, specUrl);
}

function transformSpec(spec: OpenAPIDocument, specUrl?: string): ParsedSpec {
	const servers = extractServers(spec, specUrl);
...
function extractServers(spec: OpenAPIDocument, specUrl?: string): ServerInfo[] {
	// Helper to resolve relative URLs
	const resolveUrl = (serverUrl: string): string => {
		// If it's already absolute, return as-is
		if (serverUrl.startsWith('http://') || serverUrl.startsWith('https://')) {
			return serverUrl;
		}

		// If we have a spec URL, resolve relative to it
		if (specUrl) {
			try {
				const base = new URL(specUrl);
				// For relative paths like "/api/v3", use the origin
				if (serverUrl.startsWith('/')) {
					return `${base.origin}${serverUrl}`;
				}
		...
		// Default fallback for relative URLs without context
		return `https://api.example.com${serverUrl.startsWith('/') ? serverUrl : `/${serverUrl}`}`;
	};
...
function extractSecuritySchemes(spec: OpenAPIDocument): Record<string, SecurityScheme> {
	// OpenAPI 3.x
