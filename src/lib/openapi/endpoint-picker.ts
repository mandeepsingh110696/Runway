...
function calculateScore(endpoint: ParsedEndpoint, patterns: RegExp[]): number {
	let score = 0;
	...
	// Endpoints with path parameters are much harder to test - heavy penalty
	const pathParams = endpoint.parameters.filter((p) => p.in === 'path').length;
	score -= pathParams * 50;

	// Endpoints with {param} in path are hard to test
	if (endpoint.path.includes('{')) score -= 100;
...
export function getAlternativeEndpoints(
	...
	return spec.endpoints
		.filter((e) => !(e.path === selected.path && e.method === selected.method))
		.slice(0, limit);
}
